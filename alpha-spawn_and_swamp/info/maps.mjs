...
const makeIdMap = (func,resObj) => {
  //let visual = new Visual(0,false)
  if(!resObj)
    resObj = {}

  resObj.idMap = new CostMatrix();
  resObj.id2AllPos = [];
  let minEmpty = 0;
  //障害物ラベル化
  for (let y = 0; y < 100; y++) {
    for (let x = 0; x < 100; x++) {
      ...
      const left = resObj.idMap.get(x - 1, y);
      const up = resObj.idMap.get(x, y - 1);

      if (up != null && up != 0) {
        //上を確認
        resObj.idMap.set(x, y, up);
        resObj.id2AllPos[up].push(pos);
        //間違えてる分を修正
        if (left != null && left != 0 && up != left) {
          ...
          const pLeft = resObj.id2AllPos[left][0];
          const pUp = resObj.id2AllPos[up][0];
          ...
          for (const change of resObj.id2AllPos[from]) {
            resObj.idMap.set(change.x, change.y, to);
          }
          resObj.id2AllPos[to] = resObj.id2AllPos[to].concat(resObj.id2AllPos[from]);
          resObj.id2AllPos[from] = null;
          minEmpty = Math.min(minEmpty, from);
          //visual.text(from,pos,{font:0.4,color:'#F00000'})
        }
      } else if (left != null && left != 0) {
        //左を確認
        resObj.idMap.set(x, y, left);
        resObj.id2AllPos[left].push(pos);
      } else {
        //最小の空いてるインデックスを探す
        let index;
        for (index = minEmpty; resObj.id2AllPos[index] != null; index++) {}
        minEmpty = index + 1;

        resObj.idMap.set(x, y, index);
        resObj.id2AllPos[index] = [pos];
...
class MapData {
  ...
  rayCast(start, dir, length) {
    ...
    for (let i = 1; i < length; i++) {
      ...
      if (id != 0) return { id: id, range: i, hit: true };
...
class WallData extends MapData {
  ...
  update() {
    ...
    makeIdMap((pos) => getTerrainAt(pos) == TERRAIN_WALL,this);
...

    {
      ...
      let y = 0;
      while (getTerrainAt({ x: 13, y: y }) == TERRAIN_WALL) y++;
    ...
    for (let id = 2; id < this.id2AllPos.length; id++) {
      //空なら戻る
      if (this.id2AllPos[id] == null) continue;
      ...
      if (length < 200 && center.x < 85 && 14 < center.x) {
        ...
        if (7 < length) this.Id4Attack.push(id);
      }
      ...
      if (length < 20)
        visual.poly(convex.concat(convex[0]), {
          opacity: 0.1,
          stroke: "#0000F0",
        });
      else
  ...
  getPointConvex(id, pos) {
    return calcPointPoly(
      pos,
      this.id2Convex[id],
      this.id2ConvexLength[id],
      this.id2ConvexTotalLength[id]
    );
  }
...
class SwampData extends MapData {
  ...
  update() {
    ...
    makeIdMap((pos) => getTerrainAt(pos) == TERRAIN_SWAMP,this);
    ...
    makeIdMap((pos) => map.get(pos.x, pos.y) != 0,this);
    ...
    for (let id = 1; id < this.id2AllPos.length; id++) {
      ...
      visual.poly(outline.concat(outline[0]), {
        opacity: 0.1,
        stroke: "#0000F0",
      });
    }
...
function calcConvex(outline) {
  ...
  outline.forEach((p, i) => {
    while (
      1 < convex.length &&
      util.cross3(convex[convex.length - 1], convex[convex.length - 2], p) <= 0
    ) {
      const cross = util.cross3(
        convex[convex.length - 1],
        convex[convex.length - 2],
        p
      );
